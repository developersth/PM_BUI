<template>
  <v-card width="100vw">
    <v-app-bar dark color="red">
      <v-toolbar-title>ข้อมูล Payment Term</v-toolbar-title>
    ...
    <PaymentTermForm ref="PaymentTermForm" @add="submitAdd" @edit="submitEdit" />
...
export default {
  ...
  methods: {
    ...
    async fetchData() {
      this.loading = true
      try {
        await api.getPaymentTerms().then((response) => {
    ...
    editItem(item) {
      ...
      this.$refs.PaymentTermForm.open('edit', item)
    },
    deleteItem(item) {
      this.currentPK = item.id
      this.confirm = true
    ...
    async submitAdd(data) {
      try {
        const result = await api.addPaymentTerm(data)
    ...
    async submitEdit(data) {
      try {
        const result = await api.editPaymentTerm(this.currentPK, data)
        if (result.data) {
          this.snackbar = {
            show: true,
            text: result.data.message,
    ...
    async submitDelete() {
      this.confirm = false
      try {
        const result = await api.deletePaymentTerm(this.currentPK)
        if (result.data) {
          this.snackbar = {
            show: true,
            text: result.data.message,
